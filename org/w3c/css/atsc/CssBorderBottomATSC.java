...
public class CssBorderBottomATSC extends CssProperty implements CssOperator {
	...
	if (width == null)
	    width = new CssBorderBottomWidthATSC();
	if (style == null)
	    style = new CssBorderBottomStyleATSC();
	if (color == null)
	    color = new CssBorderBottomColorATSC();
    }
	...
	if (resolve) {
	    return ((ATSCStyle) style).getBorderBottomATSC();
	} else {
	    return ((ATSCStyle) style).cssBorderATSC.getBottom();
	}
    }
